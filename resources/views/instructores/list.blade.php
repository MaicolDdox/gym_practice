...
@section('content')
    <div class="min-h-screen bg-gradient-to-br from-gray-50 via-slate-100 to-gray-100 relative">
        ...
        <div class="absolute inset-0 overflow-hidden pointer-events-none">
            ...
            <div class="absolute bottom-20 right-10 w-96 h-96 bg-gray-400/5 rounded-full blur-3xl animate-pulse delay-1000"></div>
            <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 bg-purple-300/5 rounded-full blur-2xl animate-pulse delay-500"></div>
        </div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full">
                        <thead class="bg-gradient-to-r from-gray-100/80 to-purple-100/60 backdrop-blur-sm">
                            <tr>
                                <th class="px-6 py-4 text-left text-sm font-semibold text-gray-700 uppercase tracking-wider border-b border-gray-200/30">
                                    Nombre</th>
                                <th class="px-6 py-4 text-left text-sm font-semibold text-gray-700 uppercase tracking-wider border-b border-gray-200/30">
                                    Correo</th>
                                <th class="px-6 py-4 text-left text-sm font-semibold text-gray-700 uppercase tracking-wider border-b border-gray-200/30">
                                ...
                                @can('instructors.edit')
                                    <th class="px-6 py-4 text-center text-sm font-semibold text-gray-700 uppercase tracking-wider border-b border-gray-200/30">
                                        Acciones</th>
                                @endcan
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-200/20">
                            @foreach ($instructors as $instructor)
                                <tr class="hover:bg-gray-50/50 transition-all duration-300 group">
                                    <td class="px-6 py-4 text-gray-800 font-medium">{{ $instructor->name }}</td>
                                    <td class="px-6 py-4 text-gray-600">{{ $instructor->email }}</td>
                                    <td class="px-6 py-4 text-gray-600">{{ $instructor->created_at->format('d/m/Y') }}</td>
        ...
        <link href="https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@400;600;700&family=DM+Sans:wght@300;400;600&display=swap" rel="stylesheet">
    </div>
@endsection
