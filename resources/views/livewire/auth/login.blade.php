<x-layouts.auth :title="__('Iniciar Sesión')">
    ...
    <div class="fixed inset-0 overflow-hidden pointer-events-none">
        ...
        <div class="absolute top-40 right-20 w-24 h-24 bg-gym-orange-light/20 rounded-full floating-animation-delayed">
        </div>
        <div class="absolute bottom-32 left-1/4 w-40 h-40 bg-white/5 rounded-full floating-animation"></div>
        <div class="absolute bottom-20 right-10 w-28 h-28 bg-gym-orange/10 rounded-full floating-animation-delayed">
        </div>
    </div>
    ...
    <div class="min-h-screen flex items-center justify-center px-4 py-12 relative z-10">
        <div class="w-full max-w-md">
            ...
            <div class="text-center mb-8 slide-in-up">
                ...
                <p class="text-white/80 font-source">Accede a tu cuenta</p>
            </div>
            ...
            <div class="glass-effect rounded-2xl p-8 shadow-2xl hover-lift slide-in-up">
                <div class="flex flex-col gap-6">
                    ...
                    <form method="POST" wire:submit="login" class="flex flex-col gap-6">
                        ...
                        <div class="space-y-2">
                            <flux:input wire:model="email" :label="__('Correo Electrónico')" type="email" required
                                ...
                                style="color: black;"
                                class="input-focus w-full px-4 py-3 bg-white/90 border border-white/30 rounded-xl text-gym-dark placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-gym-orange focus:border-transparent transition-all duration-300" />
                        </div>
                        ...
                        <div class="flex items-center">
                            <flux:checkbox wire:model="remember" :label="__('Recordarme')"
                                class="text-gym-orange focus:ring-gym-orange" />
                        </div>

                        <!-- Login Button -->
                        <div class="flex items-center justify-end">
                            <flux:button variant="primary" type="submit"
                                class="w-full bg-gradient-to-r from-gym-orange to-gym-orange-light hover:from-gym-orange-dark hover:to-gym-orange text-white font-semibold py-4 px-6 rounded-xl shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-300 pulse-glow">
                    ...
                    @if (Route::has('register'))
                        <div class="space-x-1 rtl:space-x-reverse text-center text-sm text-white/80">
                            <span>{{ __('¿No tienes una cuenta?') }}</span>
                            <flux:link :href="route('login')" wire:navigate
                                class="text-gym-orange-light hover:text-white font-semibold transition-colors duration-300 hover:underline">
                                {{ __('Regístrate') }}
                            </flux:link>
                        </div>
                    @endif
